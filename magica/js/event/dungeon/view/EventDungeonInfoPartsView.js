define("underscore backbone backboneCommon ajaxControl command js/event/dungeon/view/EventDungeonCpCureView text!css/event/dungeon/EventDungeonInfoParts.css text!template/event/dungeon/EventDungeonInfoParts.html".split(" "), function(p, q, a, t, w, r, u, v)
{
  var f;
  return q.View.extend(
  {
    ...
    {
      var b = {};
      b[a.cgti + " .cpPlusBtn"] = this.curePop;
      return b
    },
    initialize: function()
    {
    ...
    render: function()
    {
      ...
      {
        model: this.model,
        userEventDungeon: this.pageJson.userEventDungeon
      }));
    ...
    checkStatus: function()
    {
      var b = a.storage.userStatusList.findWhere(
        ...
        n = this.el.getElementsByClassName("cpPointWrap")[0].getElementsByTagName("li");
      if (!(this.el.getElementsByClassName("cpPointWrap")[0].getElementsByClassName("on").length >= g.point))
      {
        for (var d = 0, h = n.length; d < h; d++) b.point > d && a.addClass(n[d], "on");
        b.point >= g.point && (f && clearInterval(f), a.addClass(this.el.getElementsByClassName("cpRemainTime")[0], "off"))
      }
    },
    timerStop: function()
    {
